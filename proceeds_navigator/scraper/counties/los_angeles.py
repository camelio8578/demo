"""Los Angeles County excess-proceeds scraper."""
...
class LosAngelesScraper(BaseScraper):
    ...
    def parse(self, html: str, source_cfg: dict, source_url: str) -> list[dict]:
        """Parse Los Angeles County HTML into raw lead dicts.

        Tries CSS selectors from config first; falls back to keyword matching.
        The LA County Treasurer and Tax Collector maintains both a dedicated
        excess-proceeds page and a defaulted secured property tax sale listing;
        both sources are handled by the same selector/fallback pattern driven
        by per-source config.
        """
